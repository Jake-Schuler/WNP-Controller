...
function previousSong() {
    Neutralino.os.execCommand("wnpcli skip-previous");
}

function playPause() {
    Neutralino.os.execCommand("wnpcli play-pause");
}
...
async function fetchData() {
    let title = await Neutralino.os.execCommand('wnpcli metadata title');
    document.getElementById("song-title").innerHTML = title.stdOut;

    let artist = await Neutralino.os.execCommand('wnpcli metadata artist');
    document.getElementById("song-artist").innerHTML = artist.stdOut;

}

async function fetchDataBackground() {
    // 10 seconds before fetching data
    await setTimeout(fetchDataBackground, 1000);
    fetchData();
}
